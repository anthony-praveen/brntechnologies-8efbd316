...
const platforms = [{
  ...
  description: "Service management platform built and operated by BRN Technologies. Demonstrates full product lifecycle ownership from idea to build to launch to operate.",
  ...
  description: "A proprietary marketing solution under development, designed to redefine how businesses approach growth, intelligence, and execution. Platform-first, data-driven architecture built for scale.",
  link: null,
...
const PlatformsSection = () => {
  ...
  return <section id="platforms" className="py-20 bg-card">
      <div className="container mx-auto px-4">
        ...
        <div className="grid md:grid-cols-3 gap-8 max-w-6xl mx-auto">
          {platforms.map((platform, index) => <div key={index} className={`rounded-xl border p-8 transition-all duration-300 flex flex-col ${platform.cardStyle || "bg-background border-border hover:border-primary/30"}`}>
              ...
              {platform.link ? <a href={platform.link} target="_blank" rel="noopener noreferrer" className="inline-block">
                  <Button variant="outline" className={`gap-2 w-full ${platform.ctaStyle || ""}`}>
                    {platform.ctaText}
                    <ExternalLink className="w-4 h-4" />
                  </Button>
                </a> : <Button onClick={scrollToContact} className="gap-2">
                  {platform.ctaText}
                </Button>}
            </div>)}
        </div>
